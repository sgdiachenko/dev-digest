...
export class RepoIntelService implements RepoIntel {
  ...
  /** Takes the narrow `RepoIntelDeps` port, not the DI container — the
   *  container satisfies it structurally, so wiring is unchanged. */
  constructor(
    private deps: RepoIntelDeps,
    db: Db,
  ) {
    this.repo = new RepoIntelRepository(db);
  }

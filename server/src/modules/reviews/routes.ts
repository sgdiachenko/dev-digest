...
/** What POST /pulls/:id/review reports back: which runs started + their reviews. */
const RunReviewResult = z.object({
  pr_id: z.string(),
  runs: z.array(
...
const FindingActionResult = z.object({ finding: FindingRecord });

export default async function reviewsRoutes(appBase: FastifyInstance) {
  const app = appBase.withTypeProvider<ZodTypeProvider>();
  const { container } = app;
  const service = container.reviewService();
  ...
  app.post(
    '/pulls/:id/review',
    {
      schema: { params: IdParams, response: { 200: RunReviewResult } },
  ...
  //
  // The ONLY route in this app without a `response` schema, on purpose: the
  // body is a text/event-stream of RunEvent frames, not a single JSON document,
  // so there is nothing for the serializer to validate. Each frame's payload is
  // already typed as `RunEvent` where it is produced.
  app.get(
    '/runs/:id/events',
  ...
  for (const action of FINDING_ACTIONS) {
    app.post(
      `/findings/:id/${action}`,
      { schema: { params: IdParams, response: { 200: FindingActionResult } } },
      async (req) => {
        const { workspaceId } = await getContext(container, req);

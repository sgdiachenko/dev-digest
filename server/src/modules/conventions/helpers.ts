 ...
 * decides which of its candidates survive, dedupe/quota, and the skill markdown
...
export interface VerifiedCandidate {
  ...
  /** The (not-yet-sanitized) probe the model suggested; frequency.ts sanitizes
   *  and possibly replaces it before it ever reaches a real grep. */
  probe: string | null;
  /** Filled in by the frequency-grounding pass; null until then / for config rows. */
 ...
 * decisions, this scan's config rules, or existing skills) or each other.
 * First occurrence wins, so the caller should sort by confidence/support
 * first — the highest-ranked one survives.
 */
...
/**
 * Cap how many candidates one category may contribute (after the caller has
 * already sorted strongest-first) so one loud pattern can't crowd out every
 * other category on the board.
 */
 ...
 * The name defaults to the literal `repo-conventions` (not a repo-slugged
 * variant) — still fully editable in the Create-skill modal before save.
 *
...
/**
 * First non-empty line of a `## anchor` section's body — used to mine rule
 * text out of an existing `type: 'convention'` skill so a re-scan doesn't
 * re-propose something already folded into a skill (Lever C).
 */
export function extractRuleLinesFromSkillBody(body: string): string[] {
  const lines = body.split('\n');
  const out: string[] = [];
  for (let i = 0; i < lines.length; i += 1) {
    if (!lines[i]!.startsWith('## ')) continue;
    const next = lines.slice(i + 1).find((l) => l.trim().length > 0);
    if (next) out.push(next.trim());
  }
  return out;
}

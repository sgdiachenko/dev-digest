...
export const ExtractionSchema = z.object({
  candidates: z.array(
    z.object({
      ...
      /**
       * A short regex-safe search string that would find OTHER instances of
       * this pattern across the whole repo (e.g. a function name, an import
       * specifier, a distinctive token). Deliberately checked and sanitized in
       * code before it ever reaches a real grep (see frequency.ts) — treat
       * this as a hint, not a trusted command.
       */
      probe: z.string(),
      /**
      ...
      /**
       * How many sampled files show the pattern. Deliberately NOT persisted as
       * confidence: it exists to make the model count before it scores. The
       * real confidence used downstream comes from a repo-wide ripgrep count
       * of `probe` (see frequency.ts), not this self-report.
       */
...
export function buildSystemPrompt(alreadyCovered: string[]): string {
  const covered =
    alreadyCovered.length > 0
      ? `\n\nALREADY COVERED — do not repeat these, a deterministic check already found them:\n${alreadyCovered.map((r) => `- ${r}`).join('\n')}`
      : '';
  return `You extract a repository's HOUSE CONVENTIONS: the unwritten rules this specific codebase follows, which a reviewer should hold new code to.
...
- probe is a short literal string (a function/symbol name, an import path, a distinctive token) that would match OTHER occurrences of this same pattern elsewhere in the repo — not a regex, just the clearest substring.
...
Use the full range — a scan where every rule lands in one category is wrong. Cover at least four different categories when the sample supports it.
...
Return at most ${MAX_CANDIDATES} candidates, strongest first. Fewer, well-grounded rules beat a long list. Returning an empty list is a valid answer.${covered}`;
}

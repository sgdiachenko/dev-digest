...
/** Skills linked to an agent, in prompt order — the Skills tab's read side. */
...
/**
 * Set/reorder an agent's linked skills. Always posts the FULL ordered set
 * (never a single link/unlink) — matches `POST /agents/:id/skills { skill_ids }`,
 * which replaces the whole set and reassigns `order` from the array index.
 * A real change (added/removed/reordered) bumps the agent's config version.
 */
export function useSetAgentSkills() {
  ...
  return useMutation({
    ...
    onSuccess: (data, { agentId }) => {
      ...
      // A skill-link change may have bumped the agent's version — refetch it.
      qc.invalidateQueries({ queryKey: ["agent", agentId] });
      qc.invalidateQueries({ queryKey: ["agents"] });
      // The Stats tab of every linked/unlinked skill needs a fresh agent_count.

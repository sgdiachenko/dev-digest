...
**2026-09-18** — `messages/<locale>/*.json` lives at the CLIENT PACKAGE ROOT (sibling of `src/`), not under `src/`, so a relative import from a component/test needs ONE MORE `../` than a same-depth import into `src/lib/*` does — e.g. a file at `src/app/skills/_components/SkillCard/` imports `lib/hooks/*` with 4×`../` but `messages/en/*.json` with 5×`../`. Easy to get wrong when copy-pasting an import line between a component and its colocated test, or between components at different tree depths — count path segments from `src/` to the file's own folder for `lib/*`, then add one more level for `messages/*`. Evidence: every new `*.test.tsx` added for L02 (`SkillCard`, `ImportSkillModal`, `StatsTab`, `VersionsTab`, agent `SkillsTab`) needed this fixed on the first typecheck/test run.
...
**2026-09-18** — `next-intl`'s message parser treats a bare `<word>` inside ANY message string as the start of a rich-text tag — not only in messages consumed via `t.rich()`. A plain `t("some.key")` call on a message containing literal `<untrusted>` (no matching `</untrusted>` anywhere in the same string) throws `IntlError: INVALID_MESSAGE: UNCLOSED_TAG` at render time and falls back to the default-locale/raw-key text. Any copy that needs to reference markup-looking text literally (e.g. explaining the server's `<untrusted>` prompt-wrapping to the user) has to avoid the angle brackets in the message itself — reword ("renders as untrusted data") rather than quote the tag syntax. Evidence: `client/messages/en/skills.json` (`import.sourceNote`), reproduced via `ImportSkillModal.test.tsx`'s stderr before the wording fix.
